...
{
	htmlIndex.open("game/index.htm");
	printHTMLhead(htmlIndex, "Jeopardy CSC1061 Game");
...
void	printHTMLpages(BinTree<Question>& questionSet, 
	                   int catCount)
{
...
void	printHTMLpage(Question item, int catCount, int& score)
{
	...
	sprintf(fileName, "game/cat%d_%d00.htm", catCount, score);
...
void	printHTMLcontent(std::ofstream & os, LinkList<Record>& categories)
{
	os << "\n<h1 align = \"center\">Backwards Jeopardy Game</h1>"
		...
		<< "\n<tr>";
  // Students: The linklist is 1 based NOT 0 based
	for (int i = 1; i <= categories.getSize(); i++) {
		// Students: LinkList must have a member function: Type LinkList<Type>::at(int);
    // Students: Record must have a member function: Record Record::upper();
    os << "\n<td align = \"center\">" << categories.at(i).upper() << "</td>";
	}
	os << "\n</tr>" << "\n<tr>";
